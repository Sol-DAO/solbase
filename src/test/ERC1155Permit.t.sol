...
contract ERC1155PermitTest is DSTestPlus {
    ...
    function testPermit() public {
        ...
        (uint8 v, bytes32 r, bytes32 s) = 
            hevm.sign(
                privateKey, 
                computeDigest(owner, to, 0, 0, block.timestamp)
            );
    ...
    function testFailPermitBadNonce() public {
        ...
        (uint8 v, bytes32 r, bytes32 s) = 
            hevm.sign(
                privateKey, 
                computeDigest(owner, to, 0, 1, block.timestamp)
            );
    ...
    function testFailPermitPastDeadline() public {
        ...
        (uint8 v, bytes32 r, bytes32 s) = hevm.sign(
            privateKey, 
            computeDigest(owner, to, 0, 0, block.timestamp - 1)
        );
    ...
    function testPermit(uint248 privateKey, uint256 id, address to) public {
        ...
        (uint8 v, bytes32 r, bytes32 s) = 
            hevm.sign(
                privateKey, 
                computeDigest(owner, to, id, 0, block.timestamp)
            );
    ...
    function testFailPermitBadNonce(uint248 privateKey, uint256 id, address to, uint256 nonce) public {
        ...
        (uint8 v, bytes32 r, bytes32 s) = 
            hevm.sign(
                privateKey, 
                computeDigest(owner, to, id, nonce, block.timestamp)
            );
    ...
    function testFailPermitPastDeadline(uint248 privateKey, uint256 id, address to) public {
        ...
        (uint8 v, bytes32 r, bytes32 s) = 
            hevm.sign(
                privateKey, 
                computeDigest(owner, to, id, 0, block.timestamp - 1)
            );
